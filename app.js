...
function showDashboard() {
    ...
    if (loggedInOperator && loggedInOperator.name) {
        ...
        if(document.getElementById('btn-admin-ai-settings')) document.getElementById('btn-admin-ai-settings').style.display = isAdmin ? 'flex' : 'none';
...
window.sendAIChatMessage = async function() {
    ...
    if(aiChatState === 'WAIT_RESOLUTION') {
        ...
        todayStart.setHours(0,0,0,0);
                ...
                try {
            const faultsSnapshot = await db.collection('faults').get();
            const allFaults = faultsSnapshot.docs.map(d => {
                let data = d.data();
                if(data.startedAt && data.startedAt.toDate) data.startedAt = data.startedAt.toDate();
            ...
            const openRef = allFaults.filter(f => f.status === 'Açık' || f.status === 'İşlemde');
            const closedRef = allFaults.filter(f => {
                if(f.status !== 'Kapalı') return false;
                if(!f.completedAt) return true;
                const d = new Date(f.completedAt);
                return d >= todayStart;
            });
            
            let contextJson = {
                acik_ve_islemde_arizalar: openRef,
                bugun_kapananlar: closedRef,
            ...
            addAIMessage("Bir veritabanı hatası oluştu: " + err.message);
            aiChatState = 'IDLE';
        }
    } else {
